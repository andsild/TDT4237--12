...
public class RatingDAO {
	public void register(Rating rRating) {
		...
		try {
			...
			String query = "INSERT INTO rating (ID, fk_customerID, fk_bookID, ratevalue) VALUES (" + "NULL, ? , ? , ?)";
	...
	public Integer getAverageRating(Integer iBookID) throws Exception {
		...
		try {
			...
			String query = "SELECT AVG(ratevalue) AS average FROM rating WHERE fk_bookID = ? " + " GROUP BY fk_bookID;";
	...
	public Integer getRating(Integer iCustomerID, Integer iBookID) {
		...
		try {
			connection = Database.getConnection("opinion");
			String query = "SELECT * FROM rating WHERE rating.fk_customerID= ? " + " AND rating.fk_bookID= ? ";
