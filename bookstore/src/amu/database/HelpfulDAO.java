...
public class HelpfulDAO {
	public void register(String sCustomerID, String sReviewID, String sHelpfulInteger) {
		...
		if (thumbExist(sCustomerID, sReviewID)) {
			...
			if (sHelpfulInteger.equals("1")) {
				sQuery = "INSERT INTO helpful (fk_reviewID, fk_customerID, thumbUP, thumbDOWN) VALUES (" + "? , ? , 1, 0);";
			} else if (sHelpfulInteger.equals("0")) {
				sQuery = "INSERT INTO helpful (fk_reviewID, fk_customerID, thumbUP, thumbDOWN) VALUES (" + " ? , ? , 0, 1);";
			} else {
	...
	public boolean thumbExist(String sCustomerID, String sReviewID) {
		...
		try {
			...
			String query = "SELECT fk_customerID, fk_reviewID FROM helpful" + " WHERE fk_customerID = ? AND fk_reviewID = ?";
