...
public class BookDAO {
	public Book findByISBN(String isbn) {
		...
		try {
			...
			String query = "SELECT * FROM book, publisher, title " + "WHERE book.isbn13 = ? AND book.title_id = title.id "
	...
	public Book findByID(String ID) {
		...
		try {
			...
			String query = "SELECT * FROM book, publisher, title " + "WHERE book.id = ? " + "AND book.title_id = title.id "
					+ "AND book.publisher_id = publisher.id;";
			statement = connection.prepareStatement(query);
	...
	public Map<String, String> getListBooks() {
		...
		try {
			...
			String sql = "SELECT title.name, book.isbn13 " + "FROM title, book, publisher "
