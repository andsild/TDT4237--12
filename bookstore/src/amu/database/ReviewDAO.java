...
public class ReviewDAO {
	public void register(Review rReview) {
		...
		try {
			...
			String query = "INSERT INTO review (ID, fk_customerID, fk_bookID, text) VALUES (" + "NULL, ? , ? , ? )";
	...
	public ArrayList<HelpfulReview> getReviews(String sBookID) {
		...
		try {
			...
			String sQuery = "SELECT R.id, R.text, SUM(COALESCE(H.thumbsUp, 0)) AS thumbsUp, SUM(COALESCE(H.thumbsDown, 0)) AS thumbsDown "
					...
					+ "FROM helpful GROUP BY fkr ) AS H ON H.fkr=R.id WHERE " + "fk_bookID = ? GROUP BY H.fkr";
			...
			while (resultSet.next()) {
				...
				int iThumbsUp = resultSet.getInt("thumbsUp"), iThumbsDown = resultSet.getInt("thumbsDown");
