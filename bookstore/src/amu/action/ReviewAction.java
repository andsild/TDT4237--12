...
public class ReviewAction implements Action {
	...
	@Override	public ActionResponse execute(HttpServletRequest request,
		...
		String sCustomerRating = request.getParameter("rating"), sIsbn = request
				.getParameter("isbn");
		...
		if (sCustomerRating != null && sIsbn != null && cCustomer != null) {
			int iRating = 0;
			try {
				double dRating = Double.parseDouble(sCustomerRating); // XXX:
																		// need
																		// validation?
				iRating = (int) dRating;
			} catch (Exception e) {
