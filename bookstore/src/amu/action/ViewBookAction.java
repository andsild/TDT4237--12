	...
	public ActionResponse execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (book != null) {
			...
			try {
				...
				request.setAttribute("reviews", new ReviewDAO().getReviews(Integer.toString(book.getId())));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		if (cCustomer != null) {
			...
			try {
				request.setAttribute("customerLists", new BookListDAO().getBooklists((cCustomer.getId())));
