	...
	public ActionResponse execute(HttpServletRequest request,
		...
		if (request.getMethod().equals("POST")) {
			...
			if (customer == null) {
				...
				reCaptcha
						.setPrivateKey("6LcwqOgSAAAAAL45ayNkEqKhWvAD7PDzFUtoBDim");
				...
				if (!reCaptchaResponse.isValid()) {
					try {
						...
						Config.VALIDATE_PASSWORD.isValid(request
								.getParameter("password"));
						Config.VALIDATE_TEXT.isValid(request
								.getParameter("name"));
					} catch (FilterUnitException e) {
						messages.put("error",
								"Sleng pikken i veggen og dans. BITCH");
					...
					customer.setPassword(CustomerDAO.hashPassword(request
							.getParameter("password")));
					customer.setActivationToken(CustomerDAO
							.generateActivationCode());
					customer = customerDAO.register(customer);


					ActionResponse actionResponse = new ActionResponse(
							ActionResponseType.REDIRECT, "activateCustomer");
